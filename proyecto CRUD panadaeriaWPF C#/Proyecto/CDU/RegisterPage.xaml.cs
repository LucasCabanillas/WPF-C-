    ...
    public partial class RegisterPage 
    {
        ...
        private void btnRegistrar_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                botonRegistrar.Contraseña = seguridad.Encrypt(txtContraseña.Password);
                ...
                using (MySqlConnection conexionBD = conexion.conexion())
                {
                    ...
                    var queryCheckDni = "SELECT COUNT(*) FROM register_login WHERE dni = @dni";
                    using (MySqlCommand cmdCheck = new MySqlCommand(queryCheckDni, conexionBD))
                    {
                        cmdCheck.Parameters.AddWithValue("@dni", botonRegistrar.Dni);
                        int count = Convert.ToInt32(cmdCheck.ExecuteScalar());

                        if (count > 0)
                        {
                            // Mostrar un mensaje de error y salir del proceso
                            System.Windows.MessageBox.Show("El DNI ingresado ya está registrado. Por favor, verifique los datos.", "Error de Validación", MessageBoxButton.OK, MessageBoxImage.Warning);
                            return; // Salir de la operación
                        }
                    }
                    //Instruccion sql
                    // Proceder con la inserción si el DNI no existe
                    var query = @"INSERT INTO register_login (dni, nombre, apellido, email, domicilio, localidad, provincia, contraseña, tipoDeContrato, rango, telefono, fechaDeNacimiento, fechaDeContratacion)  VALUES (@dni, @nombre, @apellido, @email, @domicilio, @localidad, @provincia, @contraseña, @tipoDeContrato, @rango, @telefono, @fechaDeNacimiento, @fechaDeContratacion)";
            ...
            catch (Exception ex){ //System.Windows.MessageBox.Show("Error al guardar los datos.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
